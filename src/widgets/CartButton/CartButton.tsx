...
const CartButton = () => {
  ...
  const totalItems = items.reduce(
    (sum: number, item) => (sum = sum + item.quantity),
    0
  );

  return (
    <button onClick={hangleGoToCart} className={s.cartButton}>
      <svg
      ...
      <span className={s.cartButtonText}>Корзина</span>
      {totalItems > 0 && (
        <span className={s.cartButtonQuantity}>{totalItems}</span>
      )}
    </button>
  );
};
